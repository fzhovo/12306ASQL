...
CREATE TABLE `train_types`(
    ...
    `carriage_num` INT COMMENT '车箱数',
    `max_speed` INT COMMENT '最高速度, 单位km/h',
    `wifi_state` TINYINT(3) DEFAULT 0 COMMENT '0没有Wifi, 1有wifi',
    `fool_carriages` VARCHAR(50) DEFAULT '' COMMENT '餐车车厢号, 用逗号分隔',
    `max_passenger` INT DEFAULT 0 COMMENT '定员数',
    `length` INT DEFAULT 0 COMMENT '列车长度, 单位m'
...
CREATE TABLE `carriage_types`(
    ...
    `hard_berth_number` INT DEFAULT 0 COMMENT '硬卧数量',
    `senior_soft_benth_number` INT(5) DEFAULT 0 COMMENT '高级软卧数量',
    `hard_seat_number` INT DEFAULT 0 COMMENT '硬座数量',
    `second_seat_number` INT DEFAULT 0 COMMENT '二等座数量',
...
CREATE TABLE `trains`(
    ...
    `train_type` INT UNSIGNED NOT NULL COMMENT '列车类型 外键',
    `state` TINYINT(3) DEFAULT 0 COMMENT '状态',
...
CREATE TABLE `candidates`(
    ...
    `passenger_number` INT NOT NULL COMMENT '候补人数',
    `passenger_id`INT UNSIGNED  NOT NULL COMMENT '乘客id',
    ...
    CONSTRAINT fk_candidate_passenger_id FOREIGN KEY (passenger_id) REFERENCES passengers(id) ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT fk_candidate_order_id FOREIGN KEY (order_id) REFERENCES orders(id) ON DELETE RESTRICT ON UPDATE CASCADE
...
CREATE TABLE `tickets`(
    ...
    `train_id` INT UNSIGNED NOT NULL COMMENT '车次id 外键',
    `outside_id` VARCHAR(100) NOT NULL COMMENT '票外部id',
